...
function toast(msg,ms=3600){const t=$('.toast'); if(!t) return; t.textContent=msg; t.style.display='block'; clearTimeout(window.__suyuToastTimer); window.__suyuToastTimer=setTimeout(()=>t.style.display='none',ms)}
...
async function apiPost(action, payload={}){
  try{
    ...
    let data; try{data=JSON.parse(text)}catch{data={ok:false,error:text||'Respuesta inválida del servidor'}}
    if(!r.ok) data.ok=false;
    if(data.error&&/UrlFetchApp|permis/i.test(data.error)){
      data.error='El backend aún no tiene permiso para conectar con PayPal o Mercado Pago. Abre Apps Script, ejecuta autorizarPasarelas() una vez, acepta permisos y vuelve a implementar la Web App.';
    }
    return data;
  }catch(e){
    console.warn('POST error:', e.message);
    return {ok:false, offline:true, error:'No se pudo conectar con el backend. Revisa la URL del Apps Script y la implementación como Web App.'};
  }
}
...
function filterProducts(){let arr=[...PRODUCTS]; const cat=params.get('cat'), q=params.get('q'); if(cat) arr=arr.filter(p=>[p.categoria,p.subcategoria,p.coleccion,p.genero,p.modelo,p.version].some(v=>String(v||'').toLowerCase()===cat.toLowerCase())); const sub=params.get('sub'); if(sub) arr=arr.filter(p=>String(p.subcategoria||'').toLowerCase().includes(sub.toLowerCase())); const tag=params.get('tag'); if(tag==='top') arr=arr.filter(p=>isYes(p.top_ventas)||isYes(p.destacado)); if(tag==='nuevo') arr=arr.filter(p=>isYes(p.nuevo)); if(q){const s=q.toLowerCase(); arr=arr.filter(p=>[p.nombre,p.sku,p.modelo,p.version,p.coleccion,(p.tags||[]).join(' ')].join(' ').toLowerCase().includes(s))} return arr}
...
async function renderHome(){await loadProducts(); const top=PRODUCTS.filter(p=>isYes(p.top_ventas)||isYes(p.destacado)).slice(0,4); const nw=PRODUCTS.filter(p=>isYes(p.nuevo)).slice(0,4); const topEl=$('#homeTop'), newEl=$('#homeNew'); if(topEl) topEl.innerHTML=(top.length?top:PRODUCTS.slice(0,4)).map(productCard).join(''); if(newEl) newEl.innerHTML=(nw.length?nw:PRODUCTS.slice(0,4)).map(productCard).join('')}
...
async function placeOrder(gateway){const items=cart(); if(!items.length){toast('Tu carrito está vacío'); return} const email=$('#email').value.trim(), name=$('#name').value.trim(), country=$('#country').value; if(!email||!name){toast('Completa tus datos de contacto'); return} const u=user(); const t=totals(); const address=[$('#address').value,$('#city').value,$('#state').value,$('#zip').value].filter(Boolean).join(', '); const payload={user_id:u?.user_id||'',email_cliente:email,pasarela:gateway,moneda:currency(),subtotal:t.sub,descuento:0,envio:t.ship,total:t.total,pais_envio:country,direccion_envio:address,telefono:$('#phone').value,notas_cliente:'',items:items.map(i=>({...i,subtotal:Number(i.precio_unitario)*Number(i.cantidad||1)}))}; toast('Creando pedido seguro...'); const r=await apiPost('createOrder',payload); if(!r.ok){toast('No se pudo registrar el pedido. Intenta nuevamente.'); return} localStorage.setItem('suyu_pending_cart', JSON.stringify(items)); localStorage.setItem('suyu_last_order', JSON.stringify({order_id:r.order_id,gateway,total:t.total,moneda:currency()})); toast('Conectando con '+gateway+'...'); const siteUrl=(C.SITE_URL||location.origin+location.pathname.replace(/[^/]*$/,'')).replace(/\/$/,''); const pay=await apiPost('createPayment',{order_id:r.order_id,pasarela:gateway,moneda:currency(),total:t.total,email_cliente:email,site_url:siteUrl}); if(pay.ok&&pay.redirect_url){location.href=pay.redirect_url}else{toast(pay.error||'No se pudo iniciar el pago.',7000)}}
async function markPaidFromUrl(){const id=params.get('order_id'), status=(params.get('status')||'').toLowerCase(), payment=params.get('payment_id')||params.get('collection_id'); const gateway=params.get('gateway')||''; if(id&&gateway.toLowerCase().includes('mercado')&&payment){await apiGet('verifyMercadoPago',{order_id:id,payment_id:payment}); setCart([]); return} if(id&&(status.includes('paid')||status.includes('approved'))){await apiPost('markOrderPaid',{order_id:id,estado:'PAGADO',payment_id:payment||'',pasarela:gateway}); setCart([])}}
...
function addTestProduct(){addToCart({product_id:'SUYU-TEST-USD-001',sku:'SUYU-TEST-USD-001',nombre:'Producto de prueba Suyu',talla:'Única',trenza:'No aplica',personalizacion_izquierda:'',personalizacion_derecha:'',precio_unitario:1,subtotal:1,imagen:'assets/img/productos/placeholder.svg',cantidad:1}); setTimeout(()=>location.href='carrito.html',700)}
